...
def test_files_land_beside_the_session_that_made_them():
    ...
    session = tmp = workspace.SESSIONS_DIR / "2026-08-24_120000.jsonl"
    spill.set_session(session)
    assert spill.target_dir().name == "2026-08-24_120000-out"
    assert spill.target_dir().parent == workspace.SESSIONS_DIR
...
def test_without_a_session_it_still_has_somewhere_to_go():
    ...
    assert spill.target_dir() == workspace.SESSIONS_DIR / "tool-output"
...
def test_paths_come_back_in_the_form_the_tools_take(tmp_path):
    path = spill.write("x")
    assert not workspace.display_path(path).startswith("/")   # relative to the project root
...
def test_read_pages_through_a_spilled_file():
    ...
    out = READ.execute({"path": workspace.display_path(path), "offset": 100, "limit": 3})
...
def test_grep_searches_a_spilled_file_directly():
    ...
    out = GREP.execute({"pattern": "FAILED", "path": workspace.display_path(path)})
...
def test_glob_accepts_a_file_root_too():
    path = spill.write("x")
    out = GLOB.execute({"pattern": "*", "path": workspace.display_path(path)})
